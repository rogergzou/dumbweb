<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        // checks for positive expense
        if ($_POST["expense"] <= 0)
        {
            apologize_expenses("You must provide a positive expense.");
        }
        ...
        else if (!empty($_POST["category_custom"]))
        {
            ...
            query("CREATE TABLE  $category (`id` INT( 10 ) NOT NULL ,`amount` DECIMAL( 65, 4 ) UNSIGNED NOT NULL DEFAULT  '0', `time` DATETIME NOT NULL, PRIMARY KEY ( time ))");
        }
        // else apologize
        else
        {
       ...
       // if (query("UPDATE expenses SET '$category' = ? WHERE id = ?", $_POST["expense"], $_SESSION["id"]) === false)
